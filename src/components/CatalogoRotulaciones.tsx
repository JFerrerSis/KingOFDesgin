      ...
      <div className="w-full max-w-7xl mx-auto h-full flex flex-col relative">
        ...
        <div className="w-full max-w-2xl mx-auto px-4 mb-6 shrink-0">
          ...
          <div className="flex gap-2 overflow-x-auto no-scrollbar">
            {categories.map((cat) => (
              <button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-6 py-2 rounded-full text-[10px] font-bold uppercase shrink-0 border-none cursor-pointer ${selectedCategory === cat ? 'bg-yellow-500 text-black' : 'bg-white/10 text-white/60'}`}>
                {cat}
              </button>
            ))}
          </div>
